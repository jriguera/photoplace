...
class Geolocate(Actions.Interface.Action, threading.Thread):
    ...
    def ini(self, *args, **kwargs):
        ...
        msg = _("Geolocating photos with mode=%(exifmode)s and time delta of "
            "%(maxdeltaseconds)s seconds ...") % self.dgettext
    ...
    def go(self, rini):
        ...
        for photo in self.geophotos:
            ...
            if photo.isGeoLocated() and not self.forcegeolocation:
                self.logger.debug(_("Photo: '%(photo)s' (%(photo_time)s) is "
                    "geolocated to (lon=%(photo_lon).8f, lat=%(photo_lat).8f, "
                    "ele=%(photo_ele).8f). Calculated coordinates ignored due "
                    "to no overwrite option.") % self.dgettext)
                self._notify_run(photo, -1)
                continue
            tracksegs = []
            for track in self.gpxdata.tracks:
                tracksegs += track.closest(photo.time, max_delta)
            if len(tracksegs) == 0:
                self.logger.warning(_("It is impossible to geolocate '%(photo)s'"
                    " taken at %(photo_time)s with time delta %(time_delta)s. "
                    "No closed points.") % self.dgettext)
                self._notify_run(photo, -2)
            else:
                min_diff = datetime.timedelta.max
                closed_point = None
                for tragseg in tracksegs:
                    point = tragseg.closest(photo.time)
                    delta = abs(point.time - photo.time)
                    if delta < min_diff:
                        min_diff = delta
                        closed_point = point
                ...
                if min_diff > max_delta:
                    self.logger.warning(_("It is impossible to geolocate '%(photo)s' "
                        "taken at %(photo_time)s with time delta %(time_delta)s."
                        " The nearest point is too far (time=%(point_time)s).") \
                        % self.dgettext)
                    ...
                    self.dgettext['photo_lat'] = photo.lat
                    self.dgettext['photo_ele'] = photo.ele
                    self.logger.debug(_("Photo: '%(photo)s' (%(photo_time)s) has "
                        "been geolocated with coordinates (lon=%(photo_lon).8f, "
                        "lat=%(photo_lat).8f, ele=%(photo_ele).8f, "
                        "time=%(point_time)s) with delta time '%(time_delta)s'.") \
                        % self.dgettext)
    ...
    def end(self, rgo):
        ...
        msg = _("%(proc_photos)s of %(num_photos)s photos have been geolocated.")
